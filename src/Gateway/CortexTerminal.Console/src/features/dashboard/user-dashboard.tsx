...
import { truncateId, formatDateTime, formatBytes, formatRelativeTime } from './utils'
      ...
      <Main>
        ...
        <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-4'>
          <Card>
            ...
            <CardContent>
              ...
              <p className='text-xs text-muted-foreground'>
                {stats ? `${stats.activeSessions} active · ${stats.detachedSessions} detached · ${stats.exitedSessions} exited` : ''}
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className='flex flex-row items-center justify-between pb-2'>
              <CardTitle className='text-sm font-medium'>
                {t('dashboard.myActiveSessions')}
              </CardTitle>
              <Activity className='h-4 w-4 text-blue-500' />
            </CardHeader>
            <CardContent>
              <div className='text-2xl font-bold'>
                {stats ? String(stats.activeSessions) : '-'}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className='flex flex-row items-center justify-between pb-2'>
              <CardTitle className='text-sm font-medium'>
                {t('dashboard.myWorkersOnline')}
              </CardTitle>
              <Server className='h-4 w-4 text-violet-500' />
            </CardHeader>
            <CardContent>
              <div className='text-2xl font-bold'>
                {stats ? `${stats.onlineWorkers} / ${stats.totalWorkers}` : '-'}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                {stats ? formatBytes(stats.bytesTransferred) : '-'}
              </div>
              <p className='text-xs text-muted-foreground'>
                {stats && stats.mostRecentSessionAtUtc
                  ? `${t('dashboard.lastActivity')}: ${formatRelativeTime(stats.mostRecentSessionAtUtc)}`
                  : t('dashboard.never')}
              </p>
            </CardContent>
          </Card>
