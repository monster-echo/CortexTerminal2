...
const encodedPayload = btoa(JSON.stringify({ sub: 'persisted-user' }))
const persistedToken = `header.${encodedPayload}.signature`
...
describe('useAuthStore', () => {
  ...
  it('persists access token so a new store instance reads it back', async () => {
    const useAuthStore = await importAuthStore()
    useAuthStore.getState().auth.setAccessToken(persistedToken)
    ...
    expect(useAuthStoreAfterReload.getState().auth.user).toEqual({
